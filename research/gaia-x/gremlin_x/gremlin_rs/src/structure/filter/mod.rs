...
pub fn with_tag(
    tags: &mut dyn Iterator<Item = AsTag>, filter: Filter<GraphElement, ElementFilter>,
...
mod test {
    ...
    pub fn test_simple_chain_filter() {
        ...
        let func3 = Box::new(|p: &Person| Some(p.name.starts_with("a"))) as Box<dyn Predicate<Person>>;
