...
use pegasus::api::function::{CompareFunction, DynIter, FilterFunction, FlatMapFunction, MapFunction};
use pegasus::api::{Count, Counter, Fold, MaxMin, OrderDirect, ReduceByKey, ReduceByKeyWith, Source, Sum};
use pegasus::api::{Filter, IntoDataflow, IterCondition, Iteration, Limit, Map, Sinkable, SubTask, Unary};
...
use pegasus_server::pb::{BinaryResource, OperatorDef};
...
impl JobParser<Traverser, Traverser> for GremlinJobCompiler {
    fn parse(
        &self, plan: &JobRequest, input: Source<Traverser>, output: ResultSink<Traverser>,
    ) -> Result<(), BuildJobError> {
        if let Some(source) = plan.source.as_ref().and_then(|s| s.src.as_ref()) {
            let src = self.udf_gen.gen_source(source, self.num_servers, self.server_index)?;
            let mut stream = src.into_dataflow()?;
            if let Some(ref plan) = req.plan {
                stream = self.install(stream, &plan.plan[..])?;
            }

            // TODO: do encode;
            unimplemented!()
        } else {
            Err("job input not found")?
        }
    }
}
