...
use crate::structure::{without_tag, Filter, IsSimple, TraverserFilter, TraverserFilterChain, ValueFilter};
...
//     fn gen_filter(self) -> DynResult<Box<dyn FilterFunction<Traverser>>> {
//         let value_filter_pb = self.single.ok_or(str_to_dyn_error("filter is not set in is step"))?;
