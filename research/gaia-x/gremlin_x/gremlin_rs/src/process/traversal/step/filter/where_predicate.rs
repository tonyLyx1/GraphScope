...
use crate::structure::{with_tag, AsTag, Details, Element, Filter, Token, TraverserFilterChain};
use crate::{str_err, DynResult, FromPb};
use pegasus::api::function::{FilterFunction, FnResult};
use std::sync::Arc;

struct WhereStep {
    pub start_key: Option<AsTag>,
...
impl FilterFunction<Traverser> for WhereStep {
    fn test(&self, input: &Traverser) -> FnResult<bool> {
        ...
        //     if let Some(t) = input.select_first(*start) {
        //         t.as_element().ok_or(str_err("invalid input for where predicate"))?
        //     } else {
        //         return Ok(false);
        //     }
        // } else {
        //     input.get_graph_element().ok_or(str_err("invalid input for where predicate"))?
        // };
...
//         let start_key =
//             if self.start_tag.is_none() { None } else { Some(AsTag::from_pb(self.start_tag.unwrap())?) };
//         let start_token_pb = self.start_token.take().ok_or(str_err("start token is none"))?;
...
//             select_tags.push(AsTag::from_pb(tag_pb)?);
//         }
