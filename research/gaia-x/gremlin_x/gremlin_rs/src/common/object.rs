...
impl PartialOrd for Object {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match self {
            ...
            Object::String(v) => other.as_str().map(|o| v.as_str().partial_cmp(o.as_ref())).unwrap_or(None),
...
impl<'a> PartialOrd for BorrowObject<'a> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match self {
            BorrowObject::Primitive(p) => other.as_primitive().map(|o| p.partial_cmp(&o)).unwrap_or(None),
            BorrowObject::String(v) => other.as_str().map(|o| (*v).partial_cmp(o.as_ref())).unwrap_or(None),
